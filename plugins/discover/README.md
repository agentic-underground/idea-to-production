# DISCOVER — Find what's worth building

![Wide dark branding banner: the "discover" wordmark and the tagline "sweep the field, kill weak early" beside a faint line-art radar-ripples motif on a deep graphic band.](diagrams/banner.png)

> *"Let's come up with a new idea."* — the spark, made disciplined.

DISCOVER is the **DISCOVERY** front door of the `idea-to-production` marketplace. Before any code
...
A scorecard built only on your say-so is a guess in a table. DISCOVER uses **web research** —
...
open questions, never silent guesses. Offline, it reasons from you and says so. Verify with `/discover:check`.
...
Like every `idea-to-production` plugin, DISCOVER holds the **three pillars** (knowledge-parity,
...
Verify your tools with **`/discover:check`**. Dual-licensed **MIT OR Apache-2.0**.
