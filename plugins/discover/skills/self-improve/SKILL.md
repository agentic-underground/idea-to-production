...
1. **Outcome feedback** — an opportunity it approved turned out weak (shipped, no one paid; the channel
   ...
   (symptom → which parameter was too weak → what would have caught it), emitted by FOUNDRY's LEARN
...
4. **Adversarial gate**: run **`/foundry:pr-review`** if the foundry plugin is installed; otherwise
...
5. **Open a PR** for the human to merge (never self-merge), so the improvement reaches every user. Follow
   the merge-governance the user has set (FOUNDRY's `pr-approval` default).
