...
#
# The flow-mcp server no longer serves an HTTP/WebSocket board: it is an MCP stdio core only,
# launched by the harness via the plugin's .mcp.json (bin/flow-mcp --mcp). There is
# therefore no daemon to start/stop and no board URL to advertise. The flow-mcp binary no
...
# prints an explanatory line and exits 0. The roadmap is read through the flow-mcp MCP verbs
