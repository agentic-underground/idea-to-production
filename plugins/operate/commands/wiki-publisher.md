---
description: Construct a professional GitHub wiki (opt-in) from PUBLISH's per-item docs → publish to the origin's .wiki.git.
---
...
**opt-in**, publish the per-item documentation child #12 (PUBLISH) produced — the docs under
...
Degrade gracefully: when `doc/articles/` is empty there is **nothing PUBLISH produced to publish** — say
