...
if mkdir -p "$STATE_DIR" 2>/dev/null && mkdir "$SENTINEL" 2>/dev/null; then
  SETUP_MSG="⬢ operate ships a roadmap MCP (flow-server). If \"what's on the roadmap\" isn't instant yet, run /operate:flow-setup to finish the one-time setup."
  SETUP_CTX=" SETUP: if you do NOT have mcp__…__flow-server__* tools, the server isn't connected — guide the user ONCE: (a) restart Claude Code if they just installed/updated operate; (b) run /mcp and approve 'flow-server' (a plugin MCP server requires this one-time approval — no setting, CLI, or flag can pre-approve it); the binary is pre-cached so it starts instantly; /operate:flow-setup gives a guided, verified walkthrough. Never nag."
fi
