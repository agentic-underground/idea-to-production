...
> The one-copy home for **how IDEATE adversarially challenges an idea** as it refines it. Referenced by
...
| **Stack-fit** | Does the brief's `LANGUAGE/STACK` map to a **registered** DELIVER value-handler? A buildable idea is one the conveyor can carry. **If the named stack has no handler, FLAG the gap here** (see *Stack-fit flag* below) — don't paper over it; an unhandled stack caught at ideation is a conversation, caught at BUILD it is a paused conveyor and a handler to author. |
...
> **WHEN an IDEA brief names a `LANGUAGE/STACK` with no DELIVER value-handler, the system SHALL flag the
> gap at ideation** — the cheapest place to catch it. Cross-check the brief's stack against the registered
> handlers (DELIVER's VALUE_HANDLER_POOL — Python · JS/TS · React · CSS · Rust · Rust-webapp · Rust-Tauri
> · FastAPI · vanilla-JS · GitHub-Actions · …). On a miss, do **not** silently accept the brief:
>
> 1. **Name the gap** plainly: "Stack `<X>` has no DELIVER value-handler — the conveyor can't yet carry it
>    natively."
> 2. **Offer the fork** (the same decision DELIVER's missing-handler gate will otherwise force later):
>    pivot to a **supported stack**, **author a new `handler-<X>`** first (governed by DELIVER's
...
>
> This is the ideation-side half of DELIVER's missing-handler pause-and-decide gate: the same gap, caught
