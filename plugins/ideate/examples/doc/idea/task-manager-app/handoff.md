...
> Agent-facing. The compact, history-free payload DELIVER ingests at its IDEA station. A fresh agent
...
## Entry criteria for DELIVER (discovery exit gate — all met)
...
- **Stack fit** — vanilla JS + `localStorage` maps to a registered DELIVER value-handler; no stack-fit
  flag fired.
...
- **Open question (deferred, not blocking)** — exact keyboard shortcut for "complete" (Enter on a
  focused task vs. a dedicated key). DELIVER decides during design; either satisfies the metric.
