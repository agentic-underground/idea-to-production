#!/usr/bin/env bash
# offer-welcome.sh — CONCIERGE SessionStart hook. The mirror image of inject-welcome.sh:
...
if [ ! -f "$WELCOME_FILE" ]; then
  ...
  MSG="👋 This repo has no welcome experience yet — I can set up a short, repo-aware greeting that routes whoever opens it to what people do here. Just ask, or run /concierge:define-welcome."
  life=""
  if [ -n "$phase" ]; then
    life="This repo is in an idea-to-production lifecycle at the ${phase} phase, so make the offer reflect what the product is BECOMING: read the small emergent artifact(s) for this phase (DISCOVER: .market-scanner/goal.md or doc/opportunities/; IDEATE: doc/idea/<slug>/ IDEA brief + any coined name; DESIGN: doc/design/mockups/<slug>/rationale.md; BUILD: doc/SUBJECT_MATTER_UNDERSTANDING.md or ROADMAP.md) and offer a greeting that names the emerging product and routes the next person to the next step. On accept, the welcome is then kept up to date automatically as phases advance. "
  fi
  CTX="CONCIERGE has no welcome experience for this repository (no .claude/welcome.md) and may offer to author one.
...
ON OPT-IN: run /concierge:define-welcome (the deeper read; it writes a phase-stamped .claude/welcome.md).
ON DECLINE for this repo: run  mkdir -p '${DECLINED}'  so you will not prompt again here. NEVER-anywhere: run  mkdir -p '${OPTOUT}' . Only ever write under ~/.claude/hook-state — never write opt-out state into the user's repository."
  ...
  stamp_line="$(grep -m1 'concierge:welcome' "$WELCOME_FILE" 2>/dev/null || true)"
