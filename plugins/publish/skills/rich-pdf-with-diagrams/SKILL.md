...
PUBLISH is **dual-engine** — Typst and LaTeX are both first-class. Pick per article:
...
5. **Record the lesson** *before* re-rendering the affected article. If you are working in the
   publish plugin's own source repository, commit the update there; otherwise surface the
