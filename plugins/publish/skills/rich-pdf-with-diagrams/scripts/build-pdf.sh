...
case "$engine" in
  typst)
    have typst || { echo "ERROR: engine=typst but 'typst' is not installed. See PREREQUISITES/30-publish.md"; exit 127; }
    echo "==> typst compile"
    typst compile "${article_name}.typ" "${article_name}.pdf"
    ;;
  latex)
    have pdflatex || { echo "ERROR: engine=latex but 'pdflatex' is not installed. See PREREQUISITES/30-publish.md"; exit 127; }
...
if [ "$raster" -eq 1 ]; then
  ...
  if have pdftoppm; then
    ...
    echo "==> --raster requested but no rasteriser (pdftoppm/gs/magick) found — see PREREQUISITES/30-publish.md"
  fi
