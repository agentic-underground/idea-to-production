...
1. **`layout-check.sh` — SVG-math (free).** A deterministic awk pass over the generator/`.svg`: horizontal
   ...
   geometry can see, at zero tokens.
2. **`raster-lint.sh` — cheap per-tile heuristics (free).** Token-free ImageMagick heuristics on the inline
   ...
   occlusion. Emits the list of **SUSPECT tiles** + an exit code.
...
> `layout-check.sh` (the free SVG-math tier — horizontal + vertical bounds, the inline-legibility rule, the aspect advisory);
> `raster-lint.sh` (the free cheap-raster tier — suspect-tile heuristics for overlap / crowding / edge-clip / occlusion). The other three lenses:
