...
description: >
  ...
  (not in git), assembles an efficient PDF + a markdown catalog in docs/internal/comfyui-experiment/, and folds the
...
metadata:
  type: experiment
  output: docs/internal/comfyui-experiment/{journal.jsonl, catalog.md, comfyui-model-survey.pdf} + knowledge/comfyui-model-guide.md
...
The survey is **done** when, for every model in `docs/internal/comfyui-experiment/manifest.json`: all five category cells
...
Resolve `SURVEY_DIR=docs/internal/comfyui-experiment`; `SCRIPTS=${CLAUDE_PLUGIN_ROOT}/skills/model-survey/scripts`. Read
...
## Outputs (in `docs/internal/comfyui-experiment/`)
...
`plugins/publish/knowledge/comfyui-model-guide.md`.
