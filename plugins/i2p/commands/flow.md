...
1. Lay out the nine phases (a cycle) — DISCOVER ▸ IDEATE ▸ DELIVER ▸ DESIGN ▸ BUILD ⇄ ASSURE ⇄ SECURE ▸
   ...
   **`deliver:roadmapper`** (`/roadmapper` authors the FLEET v2 `docs/roadmap/` pipeline: intake →
   EARS/feature → dependency-ordered EPIC/PLAN decomposition) plus the external **FLEET
   continuous-delivery engine** that drains it. ASSURE (deliver, quality) and SECURE (secure,
...
2. Mark which stages are **dark** (plugin not installed) and what installing it would unlock — including
   DELIVER (owned by **`deliver:roadmapper`** — `/roadmapper`; the external FLEET `pipeline` plugin
   supplies the build engine — `/pipeline:status`, `/pipeline:run`; mark it dark when `deliver` is not
...
Ground the description in `plugins/deliver/VALUE_FLOW.md` and the marketplace `README.md` composition
