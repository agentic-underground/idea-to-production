...
1. Enumerate which `idea-to-production` plugins are **currently active** (discover, ideate,
   deliver, design, security, publish, operate, and i2p itself) — judge from the skills/commands available to
...
2. Render the **three pillars** in one line each, then a compact map of the value flow
   ...
   **DELIVER** between IDEATE and DESIGN, owned by `deliver:roadmapper` (+ the external FLEET engine), and draw
...
4. Point to deeper docs: the marketplace `README.md`, `plugins/deliver/knowledge/glossary.md`, and
   `plugins/deliver/VALUE_FLOW.md`.
