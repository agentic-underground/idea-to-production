...
| ③ | **DELIVER** `[DELIVER]` | `deliver:roadmapper` (+ external FLEET engine) | turn the IDEA package into the FLEET v2 pipeline: intake → EARS/feature authoring → decomposition into atomic, dependency-ordered build-ready EPIC/PLAN slices; the FLEET engine drains it | NPD planning · PLM **plan** · Kahn topological work-ordering · agile backlog decomposition (INVEST, vertical slices) |
| ④ | **DESIGN** `[DESIGN]` | design | make the experience usable, elegant, accessible — **DESIGN gates BUILD**: design reviews surfaces as part of DELIVER intake; it also **cross-cuts during BUILD** via the frontend design system | Double Diamond **Develop** (diverge: solution) · design thinking *ideate/prototype* |
| ⑤ | **BUILD** `[BUILD]` | deliver | realise it test-first through the value conveyor (IDEA▶…▶SHIP) — *loop entry* | Double Diamond **Deliver** · NPD development · PLM **Realize** |
| ⑥ | **ASSURE** `[ASSURE]` | deliver | **certify quality** — adversarial V&V: tests green, coverage density, perf-delta, regression, architecture; *fail → re-enter BUILD* | Verification & Validation · quality gate (PDCA *check* / DMAIC *control*) |
| ⑦ | **SECURE** `[SECURE]` | secure | **certify security** — PII, secrets, supply-chain clear before exposure; *fail → re-enter BUILD*, *pass (all three) → exit loop to PUBLISH* | secure-by-design · supply-chain integrity · the security gate |
...
- **Owner** — **`deliver:roadmapper`** (authoring — the skill that captures intake, authors EARS specs,
...
- **Usability (DESIGN, design)** — present from IDEATE (you shape the experience as you define the concept).
  DESIGN has a **primary gate before BUILD** — design reviews surfaces against the design-fitness rubric as part
  of DELIVER intake, before the engine starts draining PLANs — *and* it **cross-cuts during BUILD**, where design
...
- IDEATE → DELIVER: a **handoff-contract-complete IDEA package** (deliver discovery exit criteria; READY from the
  independent challenger).
- DELIVER → DESIGN: a **decomposed, dependency-ordered set of build-ready items** (EARS-specified, topologically
  ordered v2 pipeline from `deliver:roadmapper`, drained by the external FLEET engine).
...
- **`/i2p:help`** explains this lifecycle and offers to **kick one off**; **`/i2p:lifecycle`** initialises
  and reports it.
- This doc is referenced by `deliver/VALUE_FLOW.md` and the glossary so the whole suite shares one spine.
