...
> `~/.claude` production environment that DELIVER was distilled *from* before being published as the
...
> [`glossary`](../knowledge/glossary.md) on *deliver vs forge*. `~/.claude` survives here as record only.
