...
> **Provenance archive.** Part of the DELIVER provenance archive (allowlisted by the inspector).
> References to the historical `~/.claude` origin environment are history, not runtime coupling.

This records FORGE-CODE artefacts retired during the DELIVER consolidation, and where any
...
- A standalone guide for deploying `code_quality` as an *independent* skill. Now that
  code-quality is a station inside the DELIVER plugin, install = enabling the plugin. The guide
