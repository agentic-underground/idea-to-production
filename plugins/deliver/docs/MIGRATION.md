# DELIVER Migration — traceability matrix (living record)

> **Provenance archive.** This document is part of the DELIVER provenance archive (allowlisted by
...
Source root: `~/.claude/` (the Forge repo). Target root: `~/.claude/plugins/deliver/`.
...
| `skills/deliver/references/definition-of-done.md` | `knowledge/protocols/definition-of-done.md` |
| `skills/deliver/references/test-policy.md` | `knowledge/testing/test-policy.md` |
| `skills/deliver/references/context-sentinel.md` | `knowledge/protocols/context-sentinel.md` |
| `skills/deliver/references/subject-matter-understanding.md` | `knowledge/orchestration/subject-matter-understanding.md` |
| `skills/deliver/references/agent-roster.md` | `knowledge/orchestration/agent-roster.md` |
| `skills/deliver/references/orchestration-loop.md` | `knowledge/orchestration/orchestration-loop.md` |
| `skills/deliver/references/tier-assignment.md` | `knowledge/orchestration/tier-assignment.md` |
| `skills/deliver/references/idea-cost-schema.md` | `knowledge/orchestration/idea-cost-schema.md` |
| `skills/deliver/references/solid-fragment.md` | `knowledge/architecture/solid-covenant.md` |
| `skills/deliver/SKILL.md` | `skills/builder/SKILL.md` |
...
| `agents/deliver-handler-{stack}.md` | `agents/handler-{stack}.md` |
| `agents/deliver-lead-engineer.md` | `agents/builder-lead.md` |
| `agents/development-system-orchestrator.md` | `agents/lifecycle-orchestrator.md` |
| `agents/deliver-reviewer.md` | `agents/reviewer.md` |
| `agents/development-system-reviewer.md` | merged into `agents/reviewer.md` (DOCUMENT-REVIEWER role) |
| `agents/deliver-daily-inspector.md` | `agents/inspector.md` |
...
|---|---|---|---|---|
| 1 | `deliver` | rename → **builder**; promote refs to `knowledge/` | `skills/builder/` | ☑ |
...
| 15 | `pii-audit` | **RELOCATED → `secure` plugin** (cross-cutting SECURITY, not value-carrying) | `plugins/secure/skills/scan-for-pii/` | ☑ §H |
...
|---|---|---|
| `plugins/deliver/skills/founder-method/` | **keep** (COO-altitude station method); reconcile refs | ☑ |
| `plugins/deliver/skills/vertical-slice/` | **keep** (slice-cutting discipline) | ☑ |
| `plugins/deliver/skills/value-station-handoff/` | **keep** (per-station input/exit contracts); cross-linked from `knowledge/protocols/handoff-schema.md` | ☑ |
| `plugins/deliver/plugin.json` | replace with `.claude-plugin/plugin.json` | ☑ |
| `plugins/deliver/examples/` | keep | ☑ |
...
| `deliver-lead-engineer` | inherit | `agents/builder-lead.md` | demote → cycle planner | ☑ |
| `development-system-orchestrator` | inherit | `agents/lifecycle-orchestrator.md` | demote → per-item runner | ☑ |
| `deliver-reviewer` | opus-4-7 | `agents/reviewer.md` | absorb dev-system-reviewer as a role | ☑ |
...
| `deliver-handler-architect` | opus-4-7 | `agents/handler-architect.md` | move; fix arch refs | ☑ |
| `deliver-handler-python` | inherit | `agents/handler-python.md` | move; fix test-policy ref | ☑ |
| `deliver-handler-js` | inherit | `agents/handler-js.md` | move; fix refs | ☑ |
| `deliver-handler-react` | inherit | `agents/handler-react.md` | move; fix philosophy ref | ☑ |
| `deliver-handler-fastapi` | inherit | `agents/handler-fastapi.md` | move; fix philosophy ref | ☑ |
| `deliver-handler-css` | inherit | `agents/handler-css.md` | move; fix philosophy ref | ☑ |
| `deliver-handler-playwright` | inherit | `agents/handler-playwright.md` | move | ☑ |
...
| `deliver-daily-inspector` | opus-4-7 | `agents/inspector.md` | move; retarget audit root | ☑ |
...
|---|---|---|
| SOLID **self-improvement covenant** (the replication fragment) | `skills/deliver/references/solid-fragment.md` (survivor), `skills/ideate/references/solid-fragment.md` (near-identical dup, `git rm`) | `knowledge/architecture/solid-covenant.md` |
...
| Commit-message format | `skills/development-system-core/references/COMMIT_MESSAGE.md` (canonical, 354 lines), deliver SKILL §394 pointer | `knowledge/protocols/commit-message.md` |
| EARS forms | `skills/roadmapper/references/ears-quick-reference.md`, embedded deliver pipeline notes | `knowledge/specs/ears.md` |
...
| Test-policy/coverage | `skills/deliver/references/test-policy.md`, code_quality coverage-commands, state-verification embedded | `knowledge/testing/test-policy.md` |
...
| phase-sensor hook in root `settings.json` | harvest → `plugins/deliver/hooks/hooks.json`; verify resolves | ☑ |
...
- **development-system-reviewer:** "Independent critical reviewer for generated SDLC documents… covering any document produced by the Development System, not just DELIVER pipeline artifacts." → becomes the `DOCUMENT-REVIEWER` role on `reviewer.md`.
...
plugins; DESIGN (`frontend`) was kept in deliver because it *is* an on-line station (6b).

| Extracted to | Components moved out of deliver | Why |
|---|---|---|
| **`secure`** (security) | `skills/pii-audit/`, `commands/pii-audit.md` | PII/secret scanning is a pre-release gate, not a value-carriage step. Expanded with scan-for-secrets, scan-dependencies, and a consolidated `/secure:scan-all`. |
| **`publish`** (publishing) | `skills/writer/`, `skills/rich-pdf-with-diagrams/` | deliver's value artefact is **markdown**; articles/diagrams/PDF are an optional enrichment. Expanded with `diagram-studio` and `/publish`. |

**Graceful-enhancement contract.** deliver no longer hard-references these by
`${CLAUDE_PLUGIN_ROOT}` path (which would break across a plugin boundary). Instead it refers to
them **by capability** — "if the `secure`/`publish` plugin is installed, hand off to its
...
- `publish`: `writer`'s former reference to deliver's
  `knowledge/protocols/commit-message.md` was replaced with a self-contained
  `skills/writer/references/commit-format.md` so publish never assumes deliver is installed.
