---
description: Adversarial review of a PR or local diff → a single verdict (PASS / NEEDS_REVISION / BLOCK). Fans out DELIVER reviewer roles prompted to refute the change; composes the SECURE plugin's scan-all when present; writes PR_REVIEW.md.
---
...
1. Gather the review packet for `$ARGUMENTS` (a PR number, a `base..head` range, or — if empty — the
   ...
   ```
2. Fan out the DELIVER `reviewer` agent in parallel across the adversarial roles relevant to the
