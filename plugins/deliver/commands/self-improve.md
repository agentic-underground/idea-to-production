---
description: Reflect on ONE marketplace element (agent/skill/command/knowledge doc) against the KAIZEN covenant + pillars and improve it — usually by self-cleaving an over-broad element into smaller single-purpose ones, or replacing restated knowledge with a reference. Applies on a branch → /deliver:pr-review → PR per merge governance (never self-merges).
---
...
4. **Apply on a branch** (or just propose, with `--dry-run`), run **`/deliver:pr-review`**, and on PASS
   **open a PR targeting `main`** per merge governance for the human to merge — never self-merge.

Distinct from `/deliver:inspect` (which audits the whole plugin). This fixes **one element, well**.
