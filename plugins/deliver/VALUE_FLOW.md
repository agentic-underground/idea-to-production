# DELIVER — The Value-Flow System

> **This is the spine.** Every station, handler, and agent in the DELIVER plugin links
> back to a section here. If a change to DELIVER cannot be reconciled with this document,
> the change is wrong or this document is out of date — resolve that before proceeding.
> DELIVER is a **production facility**: it does not store configuration, it carries value.

> **Marketplace spine.** DELIVER owns the **BUILD** phase *and* the **ASSURE** quality gate of the
...
> deliver:roadmapper + the external FLEET engine, sits between IDEATE and DESIGN; the **BUILD ⇄ ASSURE ⇄ SECURE** realisation loop
> sends a failed ASSURE/SECURE gate back to BUILD; OPERATE loops back to DISCOVER). ASSURE (quality
> V&V, deliver) and SECURE (security, the secure plugin) are separate first-class gates. The canonical lifecycle
> model is [`../i2p/knowledge/product-lifecycle.md`](../i2p/knowledge/product-lifecycle.md).

DELIVER is one cohesive capability: a **conveyor that carries VALUE from IDEA to
...
![The DELIVER conveyor, three layers: THE IDEA above the line (the user ⇄ IDEATE / PRODUCT_MANAGER, knowledge-parity before build) feeds a clear agent-readable brief DOWN into THE CONVEYOR, whose nine value-stations carry value left→right — IDEA (ideate) ▶ ROADMAP (roadmapper) ▶ PLAN ▶ EARS (spec) ▶ FEATURE ▶ TEST (tests) ▶ IMPLEMENT +DESIGN (builder/handlers) ▶ STORY ▶ SHIP (deliver) — with cross-cutting GOVERNANCE (reviewers · perf-delta gates · inspector, in deliver) and companions when installed (PUBLISHING → publish · SECURITY → secure · DESIGN → design) riding the whole carriage; questions flow UP, value flows DOWN.](diagrams/02-conveyor.png)
...
already challenged to knowledge-parity. DELIVER's IDEA station then **receives that package by
...
built into deliver; three are **companion plugins** that deliver uses *if installed* and degrades
cleanly without (**graceful enhancement** — deliver's value artefact is markdown):

- **GOVERNANCE** *(in deliver)* — `code-quality`, `reviewer-gate`; staffed by `reviewer`
  ...
  every transition.
- **PUBLISHING** *(companion: `publish` plugin)* — when installed, deliver hands off to its
  ...
  adversarial print/data-viz design review. When absent, DELIVER emits markdown and notes that
...
- **SECURITY** *(companion: `secure` plugin)* — when installed, deliver's release path can run
  `/secure:scan-all` (PII + secret + dependency audit → `SECURITY-REPORT.md`, verdict
  PASS/REVIEW/BLOCK) before DELIVERY. When absent, the gate is skipped with a noted recommendation
  to install `secure`.
- **DESIGN** *(companion: `design` plugin)* — when installed, deliver's rendered UI surfaces can be
  ...
  deliver's source-level `frontend` design-system (the DESIGN station 6b) by capability — extending the
...
- **`inspector`** — audits DELIVER itself; finds drift, gaps, and improvable patterns.
...
Passing context to a model is a potentially wasteful operation. DELIVER optimises for
...
builder-lead (cycle planner)       — ingests ROADMAP, builds SMU, decomposes + tiers items,
   │                                  budgets tokens, emits DELIVER_PLAN.md. Plans; doesn't run.
   ▼
...
| SECURITY *(companion)* | `secure` plugin: `scan-for-pii`, `scan-for-secrets`, `scan-dependencies` (via `/secure:scan-all`) | (parallel audit sub-agents) | — |
...
the **deliver vs forge vs founder** distinction) and draws the conceptual-domain tree. Start at the
