...
A value-handler is a DELIVER production asset that turns a *class* of IDEA into PRODUCT. When the conveyor
...
| 3 | **The KAIZEN covenant** | The handler carries the replication covenant ([`../architecture/kaizen-covenant.md`](../architecture/kaizen-covenant.md)) — like every other DELIVER artefact, it improves itself and stays in sync. | A handler that cannot learn rots; the covenant is how every future build of this stack gets calmer. |
...
4. **REVIEW** — run the adversarial gate (`/deliver:pr-review`) prompted to *refute* that the handler is
...
publish's PDF path (raised via `/operate:gemba` → publish). This doc governs *DELIVER
