...
> The one-copy home for **what DELIVER does when a required VALUE_HANDLER is absent**. It is the governing
...
risk. **This disclosure SHALL be written into `docs/internal/DELIVER_PLAN.md`** under a
...
flags an IDEA brief whose `LANGUAGE/STACK` field names a stack with no DELIVER value-handler — so the gap
surfaces as a conversation, not a paused build. A gap *accepted* at ideation becomes the same 3-way gate
when the item reaches DELIVER; a gap *missed* at ideation is the rework this axis exists to prevent.
