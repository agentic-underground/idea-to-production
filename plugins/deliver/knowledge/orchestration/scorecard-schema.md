...
| `cost.*` | **last record of `IDEA_COST.jsonl`** | **only a real DELIVER cycle records tokens** — omitted (null + note) otherwise; a shell script cannot measure tokens |
...
## MARKETPLACE scorecard — `.deliver/MARKETPLACE_SCORECARD.jsonl` (append-only)
...
| `integrity.portability_violations` | ILLEGITIMATE live `~/.claude` couplings — refs in agents/hooks/commands/skills that should resolve via `${CLAUDE_PLUGIN_ROOT}` (excludes the i2p status line + welcome hooks, i2p/global-state paths, and policy/self-reference prose, which legitimately use `~/.claude`) | → **0** |
