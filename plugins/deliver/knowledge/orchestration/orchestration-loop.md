...
> For DELIVER DEV_SYSTEM and `lifecycle-orchestrator`. Defines the loop
...
- DELIVER manages tier assignment, parallelization, and token budgeting across items
- This orchestration loop handles the per-item sequential SDLC execution
- DELIVER sentinels wrap DS sentinels in the same chain
- On `DELIVERY_COMPLETE` (with prior `STORY_PROVEN` in the chain), DELIVER records `IDEA_COST.jsonl` and marks the item done in the tier summary
