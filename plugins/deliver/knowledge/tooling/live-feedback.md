...
> Canonical reference for the **feedback musculature** available to DELIVER value-handlers:
...
>
> Reachability of every tool named here is verified by `/deliver:check`
...
DELIVER does **not** ship a browser MCP — it relies on the **host-provided `chrome-devtools` MCP**
...
absent they degrade gracefully (the test contract below still holds). *(History: DELIVER/DESIGN once
...
(`.claude-plugin/marketplace.json`, deliver entry),
giving agents real go-to-definition, hover types, and **live diagnostics** instead of grep-only
navigation. The **deliver plugin entry** carries `strict: false` (set once at the plugin-entry level
in `marketplace.json`, not per server), so a missing LSP binary degrades gracefully. DELIVER wires:
...
to DELIVER's bundled `lspServers` and is the right move for languages DELIVER does not pin.
