...
DELIVER (deliver:roadmapper + FLEET engine) → DESIGN (design) → BUILD ⇄ ASSURE ⇄ SECURE (deliver build,
deliver quality, security — the realisation loop) → PUBLISH (publish) → OPERATE (operate) ↻**
(a failed ASSURE/SECURE gate sends work back to BUILD; OPERATE loops back to DISCOVER). Three concerns
cross-cut every phase — usability (design), quality (deliver), security (the secure plugin).

---

## 2. deliver vs forge vs founder — read this once

These are easily confused — similar-sounding names for different things:

| Name | What it is | Status |
|---|---|---|
| **`deliver`** | The core **plugin** — the value-flow production system (the conveyor). | A plugin in this marketplace. |
| **DELIVER** | The same system, written as a proper noun in prose. | = the `deliver` plugin. |
| **`founder`** | The **COO orchestrator agent** ([`../agents/founder.md`](../agents/founder.md)) — turns an idea into a value-stationed path and staffs the line. A *founder* runs *DELIVER* (the production line). | An agent inside deliver. |
| **`founder-method`** | The **skill** holding the station model, discovery protocol, and test contract that `founder` follows. | A skill inside deliver. |
| **`builder` / `builder-lead`** | The orchestration **skill** (`builder`) and the **cycle-planner agent** (`builder-lead`) one altitude below `founder`. | Skill + agent inside deliver. |
...
| **`frontend`** (vs `design`) | A **skill inside deliver** — the *source-level* design system (the `@front-end` INTENT markers, `definition-of-good`, the build-time `design-critic`). `design` reviews the *rendered experience* and carries the deeper canon; it **composes with** `frontend` by capability, never duplicates it. | A skill inside deliver. |

**Rule of thumb:** if a document inside a plugin says "the FORGE" as if it were *this* system, that
is a bug — it should say **DELIVER** (the plugin) or name the specific companion. The `inspector`
audits for exactly this (`../agents/inspector.md`). And don't confuse **`design`** (the design *plugin*,
rendered-experience review) with deliver's **`frontend`** *skill* (source-level design system).
...
- **product lifecycle** — the *creation arc* the suite is organised around: **nine phases forming a
  ...
  decomposition, owner deliver:roadmapper + FLEET engine) sits between IDEATE and DESIGN. **ASSURE** (quality V&V,
  deliver) and **SECURE** (security, the secure plugin) are separate first-class gates; three concerns
  cross-cut every phase — usability (design),
  quality (deliver), security (the secure plugin). The canonical model (owners, academic lineage, entry/exit
...
- **DELIVER** (lifecycle phase ③; owner deliver:roadmapper + the external FLEET engine) — the **planning** phase between IDEATE
...
- **ASSURE** (lifecycle phase ⑥; gate, owner deliver) — the **quality** certification gate: adversarial
...
- **i2p** — the marketplace **front door / meta-layer & arrival layer**: marketplace-level meta-commands
  (`/i2p:help`, `/i2p:review`, `/i2p:check`, `/i2p:flow`) plus session-start onboarding, and the **repo
...
- **i2p / discover / ideate / deliver / secure / publish / design / operate** — the front door
  ...
  (BUILD + the ASSURE quality gate, **and** DELIVER via deliver:roadmapper authoring + the external FLEET
...
### Orchestration agents (deliver)
- **founder** — COO/portfolio orchestrator. - **builder-lead** — cycle planner (emits `DELIVER_PLAN.md`).
...
### Value-handlers (deliver · staff stations by stack)
...
### Governance agents (deliver)
- **reviewer** — adversarial gate panel, role-parametrised (EARS/SMU/BDD/COVERAGE/TEST-DESIGN/
  DESIGN/SECURITY/REGRESSION/PERFORMANCE/ARCHITECTURE/DOCUMENT). - **inspector** — audits the DELIVER
...
- **builder** (orchestrator) · **founder-method** · **vertical-slice** · **value-station-handoff** ·
  **development-system-core** · **lifecycle-states** · **ideate** (IDEA) · **roadmapper** (ROADMAP) ·
...
- **motion-language.md** — the element-level diagramming/animation language: a named-element registry
  ...
  *resolves*, halo *attention-pulses*) — the sibling of the frame-level Motion canon (it answers "what KIND of
  ...
  hold?"). Owned by **handler-composite**.
- **diagram-primitives.sh** — the shared **crafted-primitive library** (`prim_node`/`prim_token`/`prim_gate`/
  ...
  generator inherits it. Names match motion-language.md's registry.
...
### Commands
- deliver: **/deliver** · **/inspect** · **/coverage-loop** · **/phase-sensor** · **/rust-webapp-rollout** ·
  **/deliver:pr-review** · **/deliver:self-improve** · **/deliver:check** · **/deliver:prerequisites**
...
### Artefacts
- **DELIVER_PLAN.md** (cycle plan) · **IDEA_COST.jsonl** (cost ledger) · **SUBJECT_MATTER_UNDERSTANDING.md**
  (SMU) · **DELIVER_INSPECTION_REPORT.md** (inspector output, written to the project) ·
