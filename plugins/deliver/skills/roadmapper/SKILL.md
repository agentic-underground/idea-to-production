...
> EPIC/PLAN kicks the FLEET engine off (§11.4), which runs DELIVER's PLAN-scope build (builder §2.5).
...
Build THIS plan by invoking DELIVER scoped to `(EPIC_MMMM, PLAN_NNNN)`: drive DEV_SYSTEM M0→M6 to a
...
> plan's Construction process" — so this section **is** the bridge that hands the slice to DELIVER.
...
> contract above is DELIVER's **PLAN-scope build behavior** (builder §2.5 + `ds-step-9-commit-push`
> engine mode) — now live. When the FLEET engine builds this PLAN it drives DELIVER scoped to
...
Use the DELIVER commit format (from `${CLAUDE_PLUGIN_ROOT}/knowledge/protocols/commit-message.md`)
...
# Push per the project's git workflow (this repo: branch → PR → merge per .deliver/governance.md;
...
discipline DELIVER uses for implementation. roadmapper is the **orchestrator of authoring, not the
...
4. **Account.** Record the authoring spend to `IDEA_COST.jsonl` (the same cost-of-record DELIVER
...
- **THE DELIVER COVERAGE FLOOR — HARD GATE:** Run coverage and confirm **100%** before proceeding to Step 7. 100% is the floor (the consequence of pinning every behaviour), not a goal; it is the definition of done. This is not optional. This is not aspirational.
...
1. Update the roadmap entry per **merge governance** ([`../../knowledge/protocols/merge-governance.md`](../../knowledge/protocols/merge-governance.md)):
   ...
   > `admin_merge:false`. `delivered` is **not** DELIVER's blocking `AWAITING MERGE` — the engine never
...
> runs its `## Construction process` (which invokes DELIVER — builder §2.5), and owns the `state`
...
> `admin_merge:false`) is **fire-and-forget**, NOT DELIVER's blocking `AWAITING MERGE`: the engine
...
- **Kick off now:** invoke the `pipeline` plugin (external FLEET marketplace plugin) — `/pipeline:run`
  ...
  order, runs its `## Construction process` (which invokes DELIVER's PLAN-scope entry — builder §2.5),
...
- **If the `pipeline` plugin is not installed:** tell the user the engine isn't available here and offer
  the standalone path (`/deliver:build` PLAN-scope on the resolved PLAN) as a manual fallback.
...
- When the inspector surfaces a ROADMAPPER finding in `DELIVER_INSPECTION_REPORT.md`
