...
# DELIVER — Self-Improve (the self-cleaving loop)
...
> **Self-improve vs inspect.** `/deliver:inspect` audits the **whole plugin** and reports a
> severity-ranked list. `/deliver:self-improve` takes **one element** and *makes it better* — usually
...
```bash
/deliver:self-improve agents/founder.md          # reflect on one element and improve/cleave it
/deliver:self-improve handler-python             # by name
/deliver:self-improve --dry-run skills/builder   # propose only; write no changes
```
...
2. Otherwise, on a **feature branch**, make the surgical change — one element's improvement, no
   unrelated edits.
3. Run **`/deliver:pr-review`** ([`../pr-review/SKILL.md`](../pr-review/SKILL.md)). A
...
This skill is itself subject to the covenant it runs. If `/deliver:self-improve` ever finds it is
