...
| Context | Reviewer |
|---|---|
| Operating in DELIVER cycle | `reviewer` with the appropriate role parameter |
| General Development System use | `reviewer` |

Prefer `reviewer` for its specialized checklists (EARS-REVIEWER, BDD-REVIEWER, etc.). Use `reviewer` when outside a DELIVER context or when a general quality review is needed.
...
- `${CLAUDE_PLUGIN_ROOT}/agents/reviewer.md` — specialized DELIVER reviewer (preferred)
