...
# DELIVER — Dependency Check

Confirms the **software musculature** DELIVER relies on is present, so a run does not discover a
...
- **required** — DELIVER's core needs it (`git`, `bash`).
...
A `✗` is **not** a failure — DELIVER degrades gracefully (e.g. no LSP → it falls back to
