...
> a station inside the DELIVER plugin — to install, enable the plugin; the standalone
...
directory) has been superseded: `code-quality` ships **inside the deliver plugin**. To use it
everywhere, install/enable the **deliver** plugin from the marketplace — there is nothing to copy by
