...
estimation-accuracy from the **last DELIVER cycle's `IDEA_COST.jsonl`** (§12 of the builder writes it;
...
- **Per-element finding retention (P2-17, marketplace).** `marketplace-score.sh` tallies inspection
  ...
  closed-loop measure `/deliver:self-improve` asserts against:
...
removal via `/deliver:self-improve`.

## When to run

At milestones: after a roadmap item reaches `STORY_PROVEN` (product), and after any `/deliver:inspect` or
