...
# Appends one event line to .deliver/MARKETPLACE_SCORECARD.jsonl and prints a summary.
#
...
#     (inspection-core.md, inspector agents, deliver policy/protocols knowledge);
...
port_viol=$(grep -rIn '~/\.claude' plugins/ 2>/dev/null \
  ...
  | grep -vE '/deliver/skills/scorecard/|/deliver/commands/scorecard\.md|/deliver/knowledge/orchestration/scorecard-schema\.md|/deliver/knowledge/glossary\.md|/deliver/skills/self-improve/SKILL\.md' \
  | wc -l | tr -d ' ')
...
echo "$line" >> .deliver/MARKETPLACE_SCORECARD.jsonl
...
echo "  → appended to .deliver/MARKETPLACE_SCORECARD.jsonl"
