...
#   IDEA_COST.jsonl (root or doc/)   → real tokens / wall-clock / regressions from the last DELIVER cycle
...
# plugins/deliver/knowledge/protocols/degraded-capabilities.md §3.3 — "Never count a non-run as a pass."
...
# ---- real cost from the last DELIVER cycle (only a real cycle can record tokens) ----
...
if [[ -n "$icost_last" ]]; then
  ...
  cost_note="no IDEA_COST.jsonl — token/wall-clock omitted (only a real DELIVER cycle records them)"
else
