...
> **Scope lock.** IDEATE has one job: take an idea and walk it to a deliverable project brief
...
> **The rich front end lives upstream.** When the **`ideate` plugin** is installed, discovery
> (`discover`) and refinement (`ideate`) happen there, and an **IDEA package** arrives already
> challenged to knowledge-parity. This skill is then the **thin fallback receiver** — it does *not*
> re-interrogate. When the `ideate` plugin is **absent**, this skill runs the full inline dialogue (§3)
...
|---|---|
| An **IDEA package** arrives from the `ideate` plugin (agent-facing: brief + SMU-seed + first slice + handoff contract) | **Ingest, don't re-ask.** Map its fields onto the brief (§4), **verify DELIVER's discovery exit criteria** are met (actionable problem, named actors, explicit scope, concrete constraints, testable success), then go straight to §5 confirmation → §6/§7 hand-off to ROADMAPPER. Carry the package's SMU-seed forward verbatim. |
| The `ideate` plugin is **absent**, user brings a raw idea | Run the **inline fallback**: §3 dialogue → §4 brief → §5/§6/§7, exactly as before. |
...
would have prevented it) routed to the `ideate` / `discover` **self-improve** intake, so every
...
IDEATE is designed to improve itself over time. Follow this protocol at the end of any
...
Every project file produced by IDEATE (§6) **must** carry the KAIZEN Replication Fragment.
...
with the KAIZEN self-improvement model initiated by IDEATE.
