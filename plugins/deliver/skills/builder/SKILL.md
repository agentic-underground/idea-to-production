...
DELIVER is the production orchestrator of the idea-to-production conveyor. It takes a fully
...
> **DELIVER orchestrates. It does not implement.**
>
> DELIVER spawns agents, sequences phases, invokes reviewers, and records costs.
...
> VALUE_HANDLER_POOL agents. DELIVER's only direct writes are to
> `docs/internal/DELIVER_PLAN.md` and `IDEA_COST.jsonl`.
>
> **Two invocation modes.** *(1) Standalone cycle* — a human runs DELIVER; DELIVER ingests a
> whole roadmap, tiers it, and writes `DELIVER_PLAN.md` (the classic path, §3–§6). *(2) Engine
> PLAN-scope* — the **FLEET continuous-delivery engine** invokes DELIVER to deliver **ONE plan** given
> `(EPIC_NNNN, PLAN_NNNN)` (§2.5). In PLAN-scope DELIVER does **not** ingest a roadmap, tier, write
> `DELIVER_PLAN.md`, or budget the window (the engine owns scheduling + budget + land); its only direct
...
> DELIVER defers to it at every implementation and review stage.
>
...
>
> **KAIZEN applies to all documents, agents, and artefacts produced by DELIVER.**
...
| "run DELIVER" / "process the roadmap" / "orchestrate" | → §3 ROADMAP INGESTION (standalone cycle) |
| "ship the backlog" / "build everything" / "start DELIVER" | → §3 ROADMAP INGESTION (standalone cycle) |
| "run a DELIVER cycle" / "full send" | → §3 ROADMAP INGESTION (standalone cycle) |
...
| "inspect DELIVER" / "run the inspector" | → §13 INSPECTION PROTOCOL |
| "add to DELIVER" / "update DELIVER knowledge" | → §14 SELF-IMPROVEMENT |
...
1. Locate `ROADMAP.md` or `doc/ROADMAP.md`. If absent, stop and tell the user:
   > "No ROADMAP.md found. DELIVER (standalone cycle) requires a populated roadmap. Run IDEATE
...
each, it invokes DELIVER to **deliver ONE plan** given `(EPIC_NNNN, PLAN_NNNN)` against the existing
...
- **`DELIVER_PLAN.md`** — roadmapper already authored the EPIC/PLAN structure; do not re-plan it.
...
**M5/M6 are re-targeted to branch-HEAD (the engine lands, not DELIVER):**
...
LEAD ENGINEER is responsible for producing `docs/internal/DELIVER_PLAN.md` containing:

```markdown
# DELIVER Plan — [project] — [date]
...
the DELIVER_PLAN marks them as parallel-safe.

```
For each TIER in [PRIMARY, SECONDARY, TERTIARY, ...]:
  1. Display tier start banner:
     ══════════════════════════════════════
     DELIVER — Tier: PRIMARY (N items)
     Token budget: ~Nk
     ══════════════════════════════════════
...
| DELIVER Phase | DS Steps | Named Agents | Model |
|---|---|---|---|
...
**Input:** All previous sentinels + test files + DELIVER_PLAN shared-infra map
...
3. **Commit + deliver** (`ds-step-9-commit-push`): stage + commit. **Standalone:** push + update roadmap
   STATUS → COMPLETE (per `.deliver/governance.md`). **Engine PLAN-scope:** stop at the branch commit —
...
silent degrade**. Note it in `docs/internal/DELIVER_PLAN.md` under `## VALUE_HANDLER_POOL Required` with a
...
`DELIVER_PLAN.md`; the BOTH path raises the gap via `/operate:gemba`, files a DEFERRED
...
**Native stack coverage today** (what DELIVER can build without authoring a new handler): Rust
...
`STORY_PROVEN` milestone, run the **`scorecard` skill** (`/deliver:scorecard`) to snapshot the project's
...
Inspection is **user-initiated only**. Say "inspect DELIVER" / "run the
...
1. Reads the installed DELIVER plugin (`${CLAUDE_PLUGIN_ROOT}`) — skills, agents, knowledge,
...
4. Produces `DELIVER_INSPECTION_REPORT.md` in the **current project** (never outside it) with
...
DELIVER carries the KAIZEN self-improvement covenant. At the end of any full
...
3. **Inspector proposals** are collated from `DELIVER_INSPECTION_REPORT.md`.
...
This document is part of the **DELIVER** skill in the idea-to-production marketplace.
All artefacts produced by DELIVER — plans, specs, feature files, test files,
...
*Skill: DELIVER — idea-to-production marketplace*
