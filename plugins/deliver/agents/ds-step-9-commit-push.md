...
- **Standalone** — a human-run DELIVER cycle (or a legacy `ROADMAP.md` project). Full publish path:
  push + PR/merge per `.deliver/governance.md`, roadmap STATUS update, etc. (the original behaviour).
...
4. **Run the always-on adversarial gate.** Invoke `/deliver:pr-review` for this change (if it has
...
5. **(Standalone) Branch on merge governance** — read `.deliver/governance.md` (absent ⇒ default `pr-approval`;
...
8. *(Reserved — the live flow board was retired with the `flow` plugin. Roadmap state on a v2 project
   is the FLEET engine's manifest `state` column; on a legacy `ROADMAP.md` project it is the STATUS
   field updated in step 7. No separate board to sync.)*

9. Update plan file: mark checklist complete, add "Completed" section with commit hash and date.
10. If `IDEA_COST.jsonl` is in use (DELIVER context), append the cost record per [`../knowledge/orchestration/idea-cost-schema.md`](../knowledge/orchestration/idea-cost-schema.md).
...
(Mode-aware — read `.deliver/governance.md`; see [`../knowledge/protocols/merge-governance.md`](../knowledge/protocols/merge-governance.md).)
...
- *(No flow-board sync — the `flow` plugin/board was retired; v2 state is the engine's manifest column, legacy state is the STATUS field.)*
...
DELIVER's cost recording is triggered by `DELIVERY_COMPLETE`, which is the
...
handoff:
  ...
  merge_mode: "pr-approval | direct-merge"   # from .deliver/governance.md
  artifacts:
    - path: "ROADMAP.md"
  ...
  quality_gates_passed:
    - "Commit created: {hash}"
    - "adversarial review (/deliver:pr-review): PASS"
    ...
    - "Roadmap state: engine manifest column (v2) / STATUS field (legacy) — no separate board"
