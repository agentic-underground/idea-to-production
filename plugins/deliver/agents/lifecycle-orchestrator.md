...
description: Orchestrates IDEATE items across SDLC stages 0-9, enforces Definition Of Done, invokes stage agents and reviewer checks, and loops until all quality gates pass. Integrates with DELIVER's PHASE_POOL by naming and sequencing the ds-step-* agents explicitly.
...
> `DELIVER_PLAN.md`. You do **not** plan the cycle or estimate budgets (that is `builder-lead`),
...
When operating inside a DELIVER cycle:
- The DELIVER orchestrator manages tier assignment, parallelization, and token budgeting
- This orchestrator handles the per-item SDLC loop (steps 0–9)
- Sentinel chain accumulates through both layers: DELIVER sentinels wrap DS sentinels
- On `DELIVERY_COMPLETE` (with prior `STORY_PROVEN` in the chain), DELIVER records `IDEA_COST.jsonl` entry
