...
```
knowledge/
├── pillars/        # the three pillars + the implementation covenant
├── token-efficiency.md
├── policy/         # model-selection (role → model)
├── specs/          # ears, bdd-gherkin
├── testing/        # test-policy (incl. perf-delta gate), coverage-commands
├── architecture/   # solid-covenant + the quality lenses
├── protocols/      # context-sentinel, handoff-schema, commit-message, definition-of-done
