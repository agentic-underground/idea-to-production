...
> For FOUNDRY DEV_SYSTEM. Each project that uses the Development System MUST create
...
## Notes For FOUNDRY Integration

When FOUNDRY is orchestrating items, the project's `DEFINITION_OF_DONE.md` is the
authority for what constitutes completion of a single item. FOUNDRY's `IDEA_COST.jsonl`
