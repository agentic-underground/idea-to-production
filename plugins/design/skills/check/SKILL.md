...
name: check
description: >
  Verify DESIGN's external tool dependencies are installed and reachable. Trigger with /design:check (or
...
DESIGN reviews with built-in vision (the `Read` tool sees PNGs — no API key) and drives live browsers via
...
MCP** drives the **system Chromium** (ONE BROWSER — design ships no MCP); the committed-snapshot crawler
