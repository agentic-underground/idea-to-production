---
description: Finish setting up the flow-server roadmap MCP — pre-cache the binary, walk the one-time approval, and verify the connection.
---

Guide the user through finishing the **flow-server MCP** setup, then verify it. The flow-server is the
roadmap MCP server mission-control ships (`render_roadmap` answers "what's on the roadmap" by local
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/flow-server/bin/flow-server-mcp" --ensure-binary
```

**Step 2 — reconcile against YOUR OWN available tools.** Look at whether you have `mcp__flow-server__*`
tools right now:

- **If you DO have them** → the server is already connected. Don't make the user do anything. Confirm it
  works by calling `render_roadmap` (or `list_items`) and show them the result ("✓ flow-server is
...
- **If you do NOT have them** → it isn't connected yet. Tell the user exactly this, briefly:
  1. **Restart Claude Code** if they just installed or updated mission-control — Claude Code only reads a
  ...
  2. **Run `/mcp`**, find **`flow-server`** (it shows as ⏸ *Pending approval*), and **approve** it.
  ...
  4. Tell them to re-run `/mission-control:flow-setup` after approving, and you'll verify the connection.
