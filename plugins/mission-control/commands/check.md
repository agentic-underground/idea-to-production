---
description: Verify MISSION-CONTROL's operate tooling (curl, jq, log/metric CLIs) is installed and reachable — a ✓/✗ table by tier (advisory; --strict to fail on a missing required tool).
---

Run the MISSION-CONTROL dependency check. Execute the script and present its ✓/✗ table, then summarise
