...
One command, one report, one verdict. OPERATE-GATE is the front door to MISSION-CONTROL: it certifies a
...
- Composes `sentinel`'s `/dependency-audit` for the maintenance lens **by capability** (only if installed);
