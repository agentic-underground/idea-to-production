#!/usr/bin/env node
/**
 * crawl.mjs — target-agnostic UI snapshot for ATELIER's /ui-review.
 *
...
if (!BASE_URL) {
  console.error('ATELIER crawl: set BASE_URL to the running app (e.g. BASE_URL=http://localhost:5173). No port is assumed.');
  process.exit(2);
}
...
async function main() {
  ...
  console.log(`ATELIER crawl — ${today}\n  base:   ${BASE_URL}\n  out:    ${OUT}\n  views:  ${VIEWPORTS.join(', ')}`);
...
main().catch(err => { console.error('\nATELIER crawl error:', err.message); process.exit(1); });
