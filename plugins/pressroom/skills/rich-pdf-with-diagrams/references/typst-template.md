...
>
> Reachability of `typst` (and the optional `dot`) is verified by `/pressroom:check`.
...
> or TeX Gyre family (see `PREREQUISITES/30-pressroom.md`). Typst falls back gracefully through the
> font list above.
