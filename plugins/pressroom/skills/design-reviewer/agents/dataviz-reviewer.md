...
When the ILLUSTRATOR hands you **two** chart options (A and B) on the same
[SPEC](../../illustrator/references/spec-schema.md) — instead of one figure to converge — run the
[A/B comparative loop](../references/ab-comparative-loop.md):

1. Score **each** option independently on the data-viz dimensions (same rubric, same gates). Check
   data-integrity (lie factor, baseline), encoding fit, colour, and the **[dark-mode contrast gate](../../illustrator/references/dark-mode-canon.md#3--contrast-gates-measurable-not-vibes)
   on BOTH the black and white card** for each.
2. Pick the winner — gates first (a gate-tripping option loses to a gate-clean one even at lower polish),
   then fitness. State *why the winner is good*, not only *why the loser failed*.
...
The single-figure procedure above is unchanged; this mode reuses the same canon and the same gates — it just
