...
The third dimension of a pre-release security pass. `scan-for-pii` checks *your data*, `scan-for-secrets`
checks *your credentials*, `scan-dependencies` checks *the code you didn't write but ship anyway* —
...
```bash
/scan-dependencies            # audit the current repo's dependencies
/scan-dependencies ./service  # audit a specific subproject
```
...
severities into the SECURITY risk scale. When the native tool is **not** available, fall back to
...
SaaS/network service, or internal-only — read `LICENSE`/`.security` policy if present, else state the
assumption):
...
- **Via `/scan-all`:** return the findings section for consolidation.
