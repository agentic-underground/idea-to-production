...
SECURITY gets sharper from two sources:
...
1. **Reflect** on the target element (a secret pattern, a PII definition, a dependency rule, the
   scan-all composition, a skill) against the covenant + pillars: is a detection missing? is a pattern
   over-broad (false positives)? does a skill do more than one thing (→ **cleave** it)?
2. **Decide**: add/tighten a pattern (`scan-for-secrets/references/SECRET-PATTERNS.md`,
   `scan-for-pii/references/PII-DEFINITION.md`) · adjust a precision boundary · cleave · reference-not-restate.
