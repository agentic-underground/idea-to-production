...
This reference defines exactly which directories and file types each `/scan-for-pii` scope covers,
...
| Scope (`/scan-for-pii <scope>`) | Agents run | Trees scanned | Skips |
|---|---|---|---|
...
by the **scan-for-secrets** skill, because bundled secrets are a real leak vector. Keep the two
concerns distinct.
