...
> fans out (and remember the L2/convergence caveats from `docs/internal/token-fairness-learnings/`).
...
token-fairness scheduler this session (issue #2 + draft PR #3 + `docs/internal/token-fairness-learnings/`).
...
- The captured-event template → `docs/internal/token-fairness-learnings/2026-06-13-workflow-fanout-cap-bypass/`
  (incident-report + proposed-solutions). The GEMBA reflex generalises exactly this shape.
- The "create a new handler" pipeline → `docs/internal/handler-build/` (haiku research → sonnet synthesis →
...
1. **Capture** into `doc/learnings/<event-slug>/{incident-report,proposed-solutions}.md` — the **exact
   shape** of `docs/internal/token-fairness-learnings/…` (that folder is the canonical template). Append a
...
- **(1) BUILD HANDLER** → author `plugins/foundry/agents/handler-<stack>.md` using the
  research→synthesis→build→review pipeline proven in `docs/internal/handler-build/`, baking in the **pinned
