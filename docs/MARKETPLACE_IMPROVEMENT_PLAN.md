...
4. **DELIVER.** The user types `/deliver:roadmapper` — which now **resolves to a real command** — and gets a `docs/roadmap/` FLEET v2 pipeline (EPIC/PLAN docs) shaped exactly like the committed **golden sample** under `references/`. (Steps 2, 8)
...
6. **BUILD.** DELIVER drains each PLAN through the 0–9 conveyor. The LEAD selects handlers via a **documented stack-detection table** (signal → handler), so an Ansible item routes to the now-registered `handler-ansible` instead of surprising the user at the missing-handler gate. (Steps 3, 4)
7. **BUILD ⇄ ASSURE ⇄ SECURE — the loop now turns.** On a built item, `/deliver:pr-review` calls `/i2p:lifecycle done ASSURE` on PASS (advancing to SECURE) or `/i2p:lifecycle fail ASSURE` on NEEDS_REVISION/BLOCK. `/secure:scan-all` calls `/i2p:lifecycle done SECURE` on PASS or `/i2p:lifecycle fail SECURE` on BLOCK. The statusline `⇄ ×N` increments on every loop-back, and a **failure-recovery callout** tells the operator exactly which report to open, what to fix, and what to re-run. The loop is traversable for the first time. (Steps 1, 9)
...
- **Plugin:** `plugins/deliver/skills/pr-review/SKILL.md`; `plugins/secure/skills/scan-all/SKILL.md`
...
- **Phase:** DELIVER
- **Plugin:** `plugins/deliver/commands/roadmapper.md` (new); reconcile `plugins/deliver/README.md:60`
- **What to build:** Add a thin `plugins/deliver/commands/roadmapper.md` that invokes the existing roadmapper skill, so `/deliver:roadmapper` resolves (the higher-fidelity option given DELIVER's centrality). Then correct `deliver/README.md:60` so it no longer claims roadmapper has "no /command of their own". (If Claude Code already auto-exposes skills as `/<plugin>:<name>`, instead state that convention once at the top of SLASH_COMMANDS and drop the README claim — pick one and make catalog match disk.)
- **Effort:** S
- **Unblocks:** Journey moment 4 — the user types the single command every DELIVER doc tells them to and it works.
- **Done when:** A user can type `/deliver:roadmapper` and the roadmapper skill runs; no DELIVER-phase doc contradicts another on how to invoke it.

### Step 3: Register the orphan handler-ansible
- **Phase:** BUILD
- **Plugin:** `plugins/deliver/skills/builder/SKILL.md` (§8 VALUE_HANDLER_POOL); `plugins/deliver/knowledge/orchestration/agent-roster.md` ("Current handlers")
- **What to build:** Register `handler-ansible` in the §8 pool table and the roster's "Current handlers" list with a spawn-when trigger (e.g. `.yml` playbooks / `roles/` / `ansible.cfg`). Alternatively, delete `plugins/deliver/agents/handler-ansible.md` if Ansible is out of scope — an on-disk-but-unregistered handler is the worst of both worlds.
- **Effort:** S
...
- **Phase:** BUILD
- **Plugin:** `plugins/deliver/skills/builder/SKILL.md` (co-located with §8); optionally `plugins/deliver/agents/handler-roadmap-decomposition.md`
...
- **Phase:** DESIGN
- **Plugin:** `plugins/i2p/knowledge/product-lifecycle.md`; `plugins/deliver/VALUE_FLOW.md`; `plugins/design/skills/mockup/SKILL.md`
...
- **Phase:** DELIVER
- **Plugin:** `plugins/deliver/skills/roadmapper/references/` or `plugins/deliver/examples/` (new sample); `README.md` + `docs/SLASH_COMMANDS.md` (DELIVER links)
...
- **Plugin:** `plugins/deliver/README.md`; `plugins/secure/README.md`; `plugins/deliver/skills/pr-review/SKILL.md` (non-PASS output); `plugins/secure/skills/scan-all/SKILL.md` (non-PASS output)
- **What to build:** Add an operator-facing failure-recovery callout (mirroring the well-specified AWAITING_MERGE prompt block) to the deliver and secure READMEs and the pr-review/scan-all non-PASS output: open the report (`PR_REVIEW.md` / `SECURITY-REPORT.md`) → address the named findings in BUILD → re-run `/pr-review` or `/scan-all` → the loop exits only when all three gates are green. Tie the message to the `lifecycle.sh fail` transition wired in Step 1 so UX and state machine agree.
- **Effort:** S
...
- **Done when:** A non-PASS verdict from `/pr-review` or `/scan-all` prints a recovery procedure naming the report, the fix-in-BUILD step, and the re-run command; the deliver/secure READMEs carry the same callout.
...
- **Plugin:** `plugins/i2p/skills/review/SKILL.md` (lines 7, 14, 43); `plugins/deliver/VALUE_FLOW.md`; `plugins/deliver/knowledge/glossary.md`; `plugins/ideate/knowledge/ideation/idea-package.md`
...
- **Plugin:** `plugins/deliver/knowledge/glossary.md:89`; `plugins/i2p/knowledge/product-lifecycle.md:87`
- **What to build:** In glossary.md:89 replace `flow` with `i2p` in the eight-specialist list and re-map DELIVER ownership to `deliver:roadmapper` + the external FLEET engine (drop "the roadmap board + intake" surface). Rewrite product-lifecycle.md:87 to name DELIVER's owner as `deliver:roadmapper` (authoring) + external FLEET continuous-delivery engine (draining). These are the last two live stale `flow` references.
- **Effort:** S
...
- **Plugin:** `scripts/verify-prereqs.sh` (or a new `deliver:inspect` lens)
...
- **Plugin:** `plugins/deliver/knowledge/protocols/handler-annotation.md` (optional rename); `VALUE_FLOW.md` + `product-lifecycle.md` (one clarifying sentence); `README.md` + `docs/SLASH_COMMANDS.md` + i2p front-door skills (citation form)
