...
A handoff brief for the **token-fairness** repo. On 2026-06-13 a 24+-agent fan-out was driven
...
This folder records what happened and proposes fixes the repo can pick up.
...
## How this was handed off

A GitHub issue + a working branch (`fix/workflow-fanout-cap-bypass`) + a draft PR were opened on the
token-fairness repo carrying the same two docs as the working brief (docs only — CI stays green; the
Rust is for the maintainers to implement against the spec).
