# 20 — SECURITY prerequisites

SECURITY's three core lenses (PII, secrets, dependencies) work on **pattern-matching + the
...
> SECURITY's scan-for-secrets also works with **zero external tools** (regex families + entropy). The
...
> **No MCP needed.** SECURITY is built entirely on local CLI scanners — the broader gaps (SBOM, IaC,
