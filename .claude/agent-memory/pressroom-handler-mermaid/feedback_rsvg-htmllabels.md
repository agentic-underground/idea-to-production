...
- Even then, **multi-line node labels using `\n` or `<br/>`/`<small>` still route through foreignObject in v11.** Use **single-line node labels** (fold sub-text in with `·`, e.g. `"DISCOVER · market-scanner"`) to force native `<text>`/`<tspan>`.
- Verify before reading PNGs: `grep -c '<foreignObject' file.svg` must be `0`; `grep '<tspan' file.svg` should show the real label text.
- Known cosmetic cost: rsvg renders DejaVu with tight inter-tspan spacing, so `· ` separators may collapse visually (`DISCOVER·market-scanner`) — readable but tight. Acceptable; do not chase it.
