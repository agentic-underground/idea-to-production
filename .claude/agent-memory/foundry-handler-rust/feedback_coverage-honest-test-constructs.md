...
When chasing the FOUNDRY 100% region (branch) floor with `cargo llvm-cov`, certain
...
- **Distinguish NEW vs pre-existing uncovered regions before chasing 100%:** `git stash` your change and
  re-run `cargo llvm-cov --lib --json`, diff the uncovered `(file,line)` set. A FOUNDRY task floor of
