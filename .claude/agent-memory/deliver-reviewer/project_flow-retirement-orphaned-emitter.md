...
where deliver's degraded-capabilities.md is canon, operate is the consumer, and the "SessionStart
...
"seven", inject-kaizen byte-identical "eight plugins", masthead DELIVER owner→deliver, 40-mcp four→
...
plugin's hook substrate (operate can't host it per heal-itself rule — likely deliver or i2p) OR
