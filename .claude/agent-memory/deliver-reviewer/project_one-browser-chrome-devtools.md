...
PR #244 "ONE BROWSER" removed the shipped `@playwright/mcp` server (deliver/.mcp.json + deleted design/.mcp.json) and migrated to the **host-provided** `chrome-devtools` MCP (unprefixed `mcp__chrome-devtools__*`).
...
- Off-fleet is gracefully degraded (design README/plugin.json/note + live-feedback all say "degrades when absent"; phase-sensor routes on mcp.chrome-devtools). Self-containment tenet is about `${CLAUDE_PLUGIN_ROOT}` PATH resolution, NOT bundling deps — host-MCP dependency doesn't violate it.
