...
The newly-stood-up `flow` plugin (DELIVER, ex-foundry flow surface, see [[flow-plugin-standup]])
...
(all but flow) and as a *command* only in foundry; `check`/`inspect` are commands in 8 plugins (all
