...
- `plugins/operate/README.md` removed `![…](../../docs/images/operate-operate.gif)` — that GIF genuinely never existed. Operate is the ONLY plugin missing its hero gif; all 7 siblings have one (i2p-frontdoor, foundry-conveyor, publish-press, atelier-critique, ideator-converge, discover-radar, sentinel-gate). Not in ledger/memory as planned → not a premature fix, but the asymmetry is the real gap.
