...
`foundry/agents/ds-step-9-commit-push.md` still has a `curl POST /api/items/{N}/status` *fallback*
(foundry-owned, not #102) — but it PREFERS MCP `post_status` and the curl path degrades gracefully
...
[[provenance-archive]] (.i2p/roadmap/done/*, .foundry/[37]*, docs/internal/*_PLAN.md keep HTTP/WS refs
